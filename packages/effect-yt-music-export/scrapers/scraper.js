...
const tracks = $$('ytmusic-responsive-list-item-renderer').map(el => {
  ...
  const flexCols = el.querySelectorAll('.secondary-flex-columns .flex-column yt-formatted-string');
  ...
  const videoId = watchHref ? new URLSearchParams(watchHref.split('?')[1]).get('v') : null;
  ...
  const coverUrl = thumbImg?.src.replace(/w\d+-h\d+(-l\d+-rj)?/, 'w500-h500') ?? null;
