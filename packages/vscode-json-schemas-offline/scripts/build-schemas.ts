...
const CATALOG_PATH = path.resolve(__dirname, '../schemastore/src/api/json/catalog.json');
const LOCAL_SCHEMAS_DIR = path.resolve(__dirname, '../schemastore/src/schemas/json');
const OUTPUT_SCHEMAS_DIR = path.resolve(__dirname, '../schemas');
const SCHEMASTORE_OUT_DIR = path.resolve(OUTPUT_SCHEMAS_DIR, 'www.schemastore.org');
const PACKAGE_JSON_PATH = path.resolve(__dirname, '../package.json');

const SCHEMASTORE_HOSTS = new Set(['www.schemastore.org', 'json.schemastore.org', 'schemastore.org']);
...
function bulkCopyLocalSchemas(): void {
  ...
  const files = fs.readdirSync(LOCAL_SCHEMAS_DIR).filter(f => f.endsWith('.json'));
...
async function downloadWithRetry(url: string, dest: string, retries = 3): Promise<boolean> {
...
async function main(): Promise<void> {
  ...
  const entries = catalog.schemas.filter(e => e.url && e.fileMatch && e.fileMatch.length > 0);
  ...
  process.stdout.write(`\r  ${entries.length}/${entries.length} processed.   \n`);
  ...
  console.log(`Generated ${jsonValidation.length} jsonValidation entries in package.json`);
}
