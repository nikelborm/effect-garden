...
**Do NOT grep/filter the lint output to check whether the errors are in YOUR files.** This is the exact mistake the user has called out more than once: after a non-zero lint run, do not try to isolate or triage "are any of these mine?" by grepping for your filenames. The whole-repo errors are not yours to chase — full stop. `bun run typecheck` passing is the ONLY gate for your change; if it's green, you're done with linting, period. Trying to prove the lint errors aren't your fault still spends the attention the user told you not to spend.
